...
const NAME = "Quinoa";
const DESCRIPTION =
  "Quinoa serves single page applications or web components (built with NodeJS: React, Angular, Vue, Lit, …)";
...
export class QwcQuinoaCard extends LitElement {
  ...
  render() {
    ...
    if (this.build_in_progress) {
      progress = html` <div class="report">
      ...
      progress = html` <div class="report">
        ...
        <vaadin-progress-bar
          value="1"
          theme="${this.build_error} ? 'error' : 'success'"
        ></vaadin-progress-bar>
      </div>`;
    } else {
      progress = html` <div class="report"></div>`;
    }
  ...
  _renderActions() {
    return html`
      ...
      >
        <vaadin-icon
          class="installIcon"
          icon="font-awesome-solid:cloud-arrow-down"
        ></vaadin-icon>
  ...
  _install() {
    ...
    this.build_error = false;
    this.result = "";
    this.jsonRpc.install().onNext((jsonRpcResponse) => {

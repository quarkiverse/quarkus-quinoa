...
const logstream = new Object();
logstream.tagName = "qwc-quinoa-log";
logstream.getAttribute = (s) => {
    return "devui-logstream"
};

/**
 * This component represent the Quinoa Node.js Log
 */
export class QwcQuinoaLog extends LitElement {

    logControl = new LogController(this);

    jsonRpc = new JsonRpc(logstream, false);
    ...
    constructor() {
        ...
        this._selectedColumns = ['0', '4', '5', '18', '19'];
    }
    ...
    _renderLevelIcon(level) {
        if (this._selectedColumns.includes('0')) {
            if (level === "warn") {
                return html`
                    <vaadin-icon icon="font-awesome-solid:circle-exclamation" class="text-warn"></vaadin-icon>`;
            } else if (level === "error") {
                return html`
                    <vaadin-icon icon="font-awesome-solid:radiation" class="text-error"></vaadin-icon>`;
            } else if (level === "info") {
                return html`
                    <vaadin-icon icon="font-awesome-solid:circle-info" class="icon-info"></vaadin-icon>`;
            } else if (level === "debug") {
                return html`
                    <vaadin-icon icon="font-awesome-solid:bug" class="text-debug"></vaadin-icon>`;
            } else {
                return html`
                    <vaadin-icon icon="font-awesome-solid:circle" class="text-normal"></vaadin-icon>`;
            }
        }
    }
    ...
    _renderSequenceNumber(sequenceNumber) {
        if (this._selectedColumns.includes('1')) {
            return html`
                <qui-badge small><span title='Sequence number'>${sequenceNumber}</span></qui-badge>`;
        }
    }
    ...
    _renderSourceClassNameFullShort(sourceClassNameFullShort) {
        if (this._selectedColumns.includes('10')) {
            return html`<span title='Source full class name (short)' class='text-source'>[${sourceClassNameFullShort}
                ]</span>`;
        }
    }
    ...
    _renderColumnsDialog() {
        return html`
            <vaadin-checkbox-group
                ...
                <vaadin-checkbox value="1" label=Sequence number
                "></vaadin-checkbox>
    ...
    _addLogEntry(entry) {
        // only care about Quinoa messages

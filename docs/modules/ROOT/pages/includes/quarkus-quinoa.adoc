...
a|icon:lock[title=Fixed at build time] [[quarkus-quinoa_quarkus-quinoa-package-manager-install-bun-version]]`link:#quarkus-quinoa_quarkus-quinoa-package-manager-install-bun-version[quarkus.quinoa.package-manager-install.bun-version]`


[.description]
--
Install and use Bun as package manager with this version. When only `bun-version` is defined (without `node-version`, `npm-version`, `yarn-version`, or `pnpm-version`), Bun will be used as both the package manager and JavaScript runtime, and NodeJS installation will be skipped entirely. Otherwise, Bun installation will be ignored and NodeJS will be used as the JavaScript runtime and the defined package manager.

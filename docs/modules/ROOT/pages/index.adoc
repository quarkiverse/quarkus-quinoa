...
Create a new Quinoa project (with a base Quinoa starter code) using the https://quarkus.io/guides/cli-tooling[Quarkus CLI]:
[source,bash]
----
quarkus create app quinoa-app io.quarkiverse.quinoa:quarkus-quinoa
----

Or just add the extension with the https://quarkus.io/guides/cli-tooling[Quarkus CLI]:
[source,bash]
----
quarkus ext add io.quarkiverse.quinoa:quarkus-quinoa
----

Or in your `pom.xml` file, add (with a valid version):
...
Create a Web UI directory in `src/main/webui` (. This directory should contain your node code with a `package.json`.

From here, either generate an application from any existing Node built Web UI framework (like React, Angular, Lit, ...). Or, you may also create your own Node built wep-app.
...
Most CI images already include NodeJS. if they don't, just make sure to install it alongside Maven (and Yarn/PNPM if needed). Then you can use it like any Maven project.
